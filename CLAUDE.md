...
**Error handling** (`internal/pkg/errorc` + [herr](https://github.com/jeremygprawira/herr)): errors are defined once as immutable classes in the `errorc` (error customized) catalog and stamped per-use — `errorc.Database.New().Internal("failed to create user").Wrap(err)`. Two surfaces: `.Public(herr.Msg(...))` for user-meaningful 4xx messages (sent to clients), `.Internal(...)`/`.Wrap(...)` for server detail (logged only — herr structurally prevents leaks). Handlers and middleware just `return err`; the central `ErrorHandler` (`internal/deliveries/http/error_handler.go`, registered as `e.HTTPErrorHandler` in `core.Setup`) renders herr's wire body `{code, message, errors[], metadata}` with `request_id` metadata and enriches the wide-event log. Validation failures return 422 via `errorc.FromValidation`. Success envelopes are unchanged (`internal/pkg/response`).
